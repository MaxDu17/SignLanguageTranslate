...
for i in range(3):
    ...
    matrix = tool.resize_image(matrix, 64, 48, "L")
    ...
    print("\tmoving right")
    matrix_right = tool.trans_hor(matrix, random.randint(1, 6))
    tool.save_image(matrix=matrix_right, path=savepath + str(i) + "_6.jpg", type="L")
    print("\tmoving left")
    matrix_left = tool.trans_hor(matrix, -1 * random.randint(1, 6))
    tool.save_image(matrix=matrix_left, path=savepath + str(i) + "_7.jpg", type="L")
    print("\tmoving up")
    matrix_up = tool.trans_vert(matrix, random.randint(1, 6))
    tool.save_image(matrix=matrix_up, path=savepath + str(i) + "_8.jpg", type="L")
    print("\tmoving down")
    matrix_down = tool.trans_hor(matrix, -1 * random.randint(1, 6))
